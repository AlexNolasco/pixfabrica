"""Tests for std-campfire-gl audio drive."""
...
from pixfabrica_std.background.campfire_gl import CampfireGL, _precompute_beat_decay
...
def test_bass_and_amp_histories_use_job_wide_range(tmp_path: Path) -> None:
    frames = [
        AudioBusFrame(
            spectrum=[0.0] * N_SPECTRUM,
            bass=0.01,
    ...
    clip = CampfireGL(id="cf", bus_select="main", sensitivity=2.0)
...
def test_stem_analyzer_drums_has_visible_bass_and_amp_swing(tmp_path: Path) -> None:
    ...
    clip = CampfireGL(id="cf", bus_select="main", sensitivity=2.5)
