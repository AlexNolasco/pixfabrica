"""Tests for std-star-nest-gl audio drive."""
...
from pixfabrica_std.background.star_nest_gl import StarNestGL, _precompute_beat_decay
...
def test_beat_decay_spans_job_and_peaks_on_beat():
    ...
    decay = _precompute_beat_decay(frames, total=5)
    assert decay.shape[0] == 5
    assert decay[1] == pytest.approx(1.5)
    assert decay[2] < decay[1]
    assert decay[4] < decay[2]
...
def test_audio_histories_use_job_wide_range(tmp_path: Path) -> None:
    ...
    clip = StarNestGL(id="sn", bus_select="main", sensitivity=2.0)
...
def test_stem_analyzer_drums_has_visible_bass_high_amp_swing(tmp_path: Path) -> None:
    ...
    clip = StarNestGL(id="sn", bus_select="main", sensitivity=2.5)
