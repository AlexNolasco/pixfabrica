"""Bounds-local GL sway — infinity-path rocking with smooth center-pivot tilt."""
...
void main() {
    float t = TAU * (u_time / u_period + u_phase);
    float t0 = u_t_at_origin;

    // Gerono lemniscate: x ~ sin(t), y ~ sin(2t)/2 — smooth figure-eight
    float sx = sin(t) - sin(t0);
    float sy = 0.5 * (sin(2.0 * t) - sin(2.0 * t0));
    vec2 drift = vec2(u_sway * sx, u_bob * sy);

    // Tilt follows the horizontal arm of the path (smooth, no skew)
    float angle = u_tilt * sx;
    ...
    vec2 uv = rotated + drift + vec2(0.5);
...
# Fixed infinity-path shape (no UI knobs).
SWAY = 0.01
BOB = 0.025


class SwayGL(GLEffect):
    """Slow infinity-path rock — center pivot, smooth tilt in sync with the curve."""

    uv_margin: ClassVar[float] = 0.03
    effect_type: ClassVar[str] = "std-sway-gl"
    ...
    tilt: float = Field(
        ...
        le=8.0,
        multiple_of=0.5,
        description="Peak rotation in degrees about center",
    )
    period: float = Field(
        default=7.0,
        ge=3.0,
        le=12.0,
        multiple_of=0.5,
        description="Seconds for one full infinity loop",
    )
    ...
    def apply(self, ctx: EffectContext) -> None:
        ...
        t_at_origin = 2.0 * math.pi * (self._time_origin / period + phase)
        tilt_rad = math.radians(float(self.tilt))
        ...
        self._set_uniform(self._program, "u_sway", SWAY)
        self._set_uniform(self._program, "u_bob", BOB)
        self._set_uniform(self._program, "u_tilt", tilt_rad)
        ...
        self._set_uniform(self._program, "u_t_at_origin", t_at_origin)
