/** Stable MD5 of JSON — mirrors ``pixfabrica_api.routes.preview._graph_hash``. */
...
/** MD5 hex digest (browser-safe, no dependency). */
function md5Hex(text: string): string {
  ...
  function toWords(input: string): number[] {
    ...
    for (let i = 0; i < length; i += 1) {
      words[i >> 2] = (words[i >> 2] ?? 0) | (input.charCodeAt(i) << ((i % 4) * 8))
    }
...
export function graphHash(graph: unknown): string {
  ...
  return md5Hex(JSON.stringify(sorted))
}
